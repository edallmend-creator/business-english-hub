...
function renderCard() {
        ...
        <div class="settings-bar" onclick="event.stopPropagation();" style="pointer-events: auto;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <div style="display: flex; gap: 8px; align-items: center; cursor: pointer;" 
                     onclick="event.stopPropagation(); const c = document.getElementById('settingsContent'); const i = document.getElementById('settingsToggleIcon'); if(c && i) { const isOpen = c.style.display !== 'none'; c.style.display = isOpen ? 'none' : 'block'; i.textContent = isOpen ? '▼' : '▲'; }">
            ...
            <div id="settingsContent" style="display: none; pointer-events: auto;" onclick="event.stopPropagation();">
                <div class="settings-toggles">
                    ...
                    <label ${!CONFIG.timer.enabled ? 'style="opacity: 0.5;" title="Ab Woche 2 verfügbar"' : ''}>
                        <input type="checkbox" 
                            ...
                            onclick="event.stopPropagation();" onchange="window.toggleSetting('timer', this.checked)">
                ...
                ${CONFIG.timer.enabled && App.settings.timer ? `
                    <div id="timerSliderContainer" style="margin-top: 16px; padding: 12px; background: var(--surface); border-radius: 8px; border: 1px solid var(--border); border-top: 2px solid var(--primary);">
                        ...
                        <input type="range" min="5" max="90" step="5" value="${App.settings.timer_duration}"
                            oninput="event.stopPropagation(); window.updateTimerDuration(this.value)"
        ...
        <div style="margin-bottom: 20px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                ...
                <div style="display: flex; gap: 12px; align-items: center;">
                    ...
                    <button onclick="exitCoach()" class="coach-exit-btn" title="Zurück zum Hub">
                        ✕
                    </button>
                </div>
            </div>
    ...
    // Render directly into the page, not into the small modal.
    const contentEl = document.getElementById('app');
    if (contentEl) {
        contentEl.innerHTML = `<main class="coach-session">${html}</main>`;
    }
...
function updateTimerDuration(value) {
    ...
    App.settings.timer_duration = numValue;
    saveSetting('timer_duration', value);
    console.log(`⏱️ Timer-Dauer: ${value}s`);
    
    // Update Timer Display (in Settings Slider)
    const timerSlider = document.querySelector('#timerSliderContainer strong');
    if (timerSlider) {
        timerSlider.textContent = numValue + 's';
    }
    
    // NICHT renderCard() aufrufen - das schließt Settings!
}
...
function showSummary() {
    ...
    if (container) {
        container.innerHTML = `<main class="coach-session coach-summary">${html}</main>`;
    }
...
function setupKeys() {
    document.onkeydown = function(e) {
        // Only handle while a coach session is active on the page.
        if (!document.querySelector('.coach-session')) {
            return;
        }
...
function showDeckSelection() {
    ...
    const deckStats = App.availableDecks.map(deck => {
        ...
        return { deck, due, mastered, total: cards.length };
    });

    const totalDue = deckStats.reduce((sum, item) => sum + item.due, 0);
    const totalMastered = deckStats.reduce((sum, item) => sum + item.mastered, 0);
    const practicedDecks = deckStats.filter(item => item.total > 0).length;
    const recommended = deckStats.find(item => item.due > 0) || deckStats.find(item => item.deck.category === 'core') || deckStats[0];
    const categoryLabels = {
        core: '4-Wochen-Pfad',
        special: 'Fokus-Training',
    ...
    const html = `
        <main class="coach-home">
            <section class="coach-home-hero">
                <div>
                    <div class="coach-kicker">Persönlicher Trainingsbereich</div>
                    <h1>Business English Coach</h1>
                    <p>Trainiere die richtigen Phrasen im richtigen Moment. Kurze Sessions, automatische Wiederholung und klare Fortschritte.</p>
                    <div class="coach-home-actions">
                        <button class="btn btn-primary" onclick="selectDeck('${recommended.deck.id}')">
                            ${recommended.due > 0 ? `Heute wiederholen (${recommended.due})` : 'Empfohlenes Training starten'}
                        </button>
                        <button class="btn btn-secondary" onclick="showGlobalDashboard()">Fortschritt ansehen</button>
                    </div>
                </div>
                <div class="coach-today-card">
                    <div class="coach-today-label">Heute sinnvoll</div>
            ...
            <section class="coach-metrics">
                <div><strong>${totalDue}</strong><span>heute fällig</span></div>
                <div><strong>${totalMastered}</strong><span>gemeistert</span></div>
                <div><strong>${practicedDecks}</strong><span>aktive Decks</span></div>
            </section>
            ...
            <section class="coach-deck-grid">
                ${deckStats.map(({ deck, due, mastered, total }) => `
                    <article class="coach-deck-card" style="--deck-color: ${deck.color};">
                        <div class="coach-deck-meta">${categoryLabels[deck.category] || 'Training'}</div>
                        <h2>${deck.name}</h2>
                        <p>${deck.description || ''}</p>
                        <div class="coach-deck-stats">
                            ${due > 0 ? `<span class="is-due">${due} fällig</span>` : `<span>bereit</span>`}
                            ${mastered > 0 ? `<span>${mastered} gemeistert</span>` : ''}
                            ${total > 0 ? `<span>${total} geübt</span>` : ''}
                        </div>
                        <div class="coach-deck-actions">
                            <button class="btn btn-primary" onclick="selectDeck('${deck.id}')">
                                ${due > 0 ? 'Review starten' : 'Lernen starten'}
                            </button>
                            <button class="btn btn-secondary" onclick="showSRSDashboard('${deck.id}')">Details</button>
                        </div>
                    </article>
                `).join('')}
            </section>
